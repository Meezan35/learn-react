...
const Header = () => {
  ...
  return (
    <header className="bg-orange-500 text-white py-4 px-8 flex justify-between items-center shadow-lg">
      <div className="flex items-center space-x-2">
        <img src={logo} alt="Food Delivery Logo" className="h-10 w-10" />
      ...
      <nav className="flex items-center space-x-8">
        ...
        <div className="relative">
          <button className="flex items-center">
            <svg className="w-6 h-6 hover:text-yellow-300 transition duration-200" fill="currentColor" viewBox="0 0 24 24">
              <path d="M3 3h2l.4 2M7 13h14l-1.5 8H6.5L7 13zm5 6a2 2 0 100-4 2 2 0 000 4zm6 0a2 2 0 100-4 2 2 0 000 4zM5 6h15l-1.68 9H6.68L5 6z"></path>
            </svg>
...
const RestaurantCard = ({restaurantData}) => {
    ...
    const {  cloudinaryImageId,
        ...
        costForTwoString,
        avgRating} = restaurantData

  return (
    <div className="bg-white shadow-lg rounded-lg p-6 space-y-4 w-[200px] h-[600px]">
      <img src={`https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_660,/${cloudinaryImageId}`} alt="Food Delivery Logo" className="10 w-10" />
      <h3 className="text-2xl font-bold text-gray-800">{name}</h3>
      <div className="text-center space-y-2">
        <h4 className="text-gray-600 text-lg">{cuisines.join(", ")}</h4>
        <h4 className="text-yellow-500 text-lg font-semibold">{avgRating}</h4>
        <h4 className="text-gray-600 text-lg">{lastMileTravelString}</h4>
      </div>
    </div>
  );
};
...
const Body = () => {
    const [restaurantList, setRestaurantList] = useState([ {
      ...
      const [filteredRes,setFilterRes] = useState(restaurantList)
      const [searchtext, setSearchText] = useState("")
      const onlineStaus = useOnlineStatus();

      if(!onlineStaus){
        return <p>oopsss looks like you are offline</p>
      }
    
  return (
    <div className="mt-8">
      <div className="flex flex-wrap gap-4 justify-start w-full max-w-screen-xl mx-auto">
        <div ><button
        className='filter-btn'
        style={{padding:"5px", backgroundColor:"grey", borderRadius:"5px",margin:"4px", color:"white"}}
        ...
        >Filter</button>
       <input type="text" placeholder="Search.." style={{border:"1px solid black"}} onChange={(e)=>setSearchText(e.target.value)}></input>
       <button
        className='filter-btn'
        style={{padding:"5px", backgroundColor:"grey", borderRadius:"5px",margin:"4px", color:"white"}}

        onClick={()=>{
            const filterList = restaurantList.filter((res)=>res.data.name.includes(searchtext))
